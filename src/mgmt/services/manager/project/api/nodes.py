...
def update_droplet(droplet):
    printlog('Updating this droplet with name {}: \n{}\n'.format(droplet['metadata']['name'], droplet))
    try:
        obj_api.replace_namespaced_custom_object(group='zeta.com',
                                                version='v1',
                                                plural='droplets',
                                                namespace='default', 
                                                name=droplet['metadata']['name'],
                                                body=droplet)
        # obj_api.replace_cluster_custom_object(group='zeta.com',
        #                                         version='v1',
        #                                         plural='droplets', name=droplet['metadata']['name'],body=droplet['spec'])
    except ApiException as e:
        printlog('Exception when updating exsiting droplets: {}'.format(e))
    
    printlog('Updated droplet: {}'.format(droplet['metadata']['name']))
...
def create_droplet(name, ip, mac, itf, network, zgc_id):
    ...
    printlog('Creating droplet using this name: {} \nbody: {}'.format(name, droplet_body))
    try:
        ...
        printlog('Exception when Creating droplets: {}'.format(e))
            ...
            if number_of_ip_new_droplet_gets > 0 : # each droplet should have 1 or more ip, assign ip / mac from exiting droplets
                ...
                while len(ip_for_new_droplet) < number_of_ip_new_droplet_gets:
                    ...
                    if current_length_of_ip_list == len(ip_for_new_droplet):
                        ...
                        current_length_of_ip_list = len(ip_for_new_droplet)
                printlog('Got enough new ips, new ips for new droplet: {}, length of new ip list is: {}, and we need: {}'.format(ip_for_new_droplet, len(ip_for_new_droplet), number_of_ip_new_droplet_gets))
                macs_for_new_droplet = [get_mac_from_ip(ip) for ip in ip_for_new_droplet]
                printlog('Also got enough macs: {}, length of mac list: {}'.format(macs_for_new_droplet, len(macs_for_new_droplet)))

                printlog('Update existing droplets first')
